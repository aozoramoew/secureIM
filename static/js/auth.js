...
async function handleRegister(e) {
  ...
  try {
    ...
    setTimeout(() => { window.location.href = '/login?registered=1'; }, 2000);
...
async function handleLogin(e) {
  ...
  try {
    ...
    if (hasKeys) {
      ...
      // Re-derive public keys from stored private keys
      const ecdsaPrivKey = await SecureCrypto.importPrivateECDH(keys.ecdhPrivJwk);
      // For display, just re-export what we have
      ecdsaPubJwk = null;
      ecdhPubJwk  = null;
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (params.get('registered') === '1') {
    setStatus('login-status', '📧 Registration successful! Check your email to verify your account.', 'success');
  }
});

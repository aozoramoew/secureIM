...
def _get_socket_user(token: str | None, environ: dict | None = None):
    """
    Decode JWT for a SocketIO connection.
    Priority:
      1. auth dict token (passed by client as io({auth:{token}}))
      2. sim_token HttpOnly cookie from the HTTP handshake environ
    Returns (user, device_id) or (None, None).
    """
    if not token and environ:
        # Parse cookies from the HTTP_COOKIE environ key
        cookie_header = environ.get('HTTP_COOKIE', '')
        for part in cookie_header.split(';'):
            part = part.strip()
            if part.startswith('sim_token='):
                token = part[len('sim_token='):]
                break
...
async def connect(sid, environ, auth):
    ...
    user, device_id = _get_socket_user(token, environ)
...
async def send_message(sid, data):
    """
    data = {
      session_id: int,           # ECDH session ID
      recipient_id: int,         # DM recipient
